...
1. Follow TDD for all implementation — the cycle is: Write failing test -> verify it FAILS -> write minimal code -> verify it PASSES -> refactor -> commit. **Never skip verifying the test fails first.** A test that passes before implementation protects nothing. **See:** `skills/implement/tdd.md` for test discovery, failure mode troubleshooting, and boundary test patterns. **Exception:** Consolidated mechanical tasks (renames, import additions, config updates across multiple files) may specify a lighter verification in their prose — e.g., "run the full test suite and confirm no regressions." Follow whatever discipline the task prose prescribes.
